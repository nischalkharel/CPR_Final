...
class ChessSetupChecker:
    def __init__(self, i2c):
        ...
        self.piece_positions = {
            "a1": "R{B}", "b1": "N{B}", "c1": "B{B}", "d1": "K{B}", "e1": "Q{B}",
    ...
    def generate_initial_chess_setup(self):
        oled = ChessOLED()
        oled.display("Initial Checks","in progress")
        error_detected = 0
        while True:
            time.sleep(0.1)
            ...
            for position, pin in self.all_pins.items():
                ...
                if expected_piece == "empty" and piece_present:
                    ...
                    if first_error == 0:
                        oled.display("Unexpected Piece on", f"{position}")
                        first_error = 1
                elif expected_piece != "empty" and not piece_present:
                    errors.append(f"{expected_piece} is not on {position} as expected")
                    if first_error == 0:
                        oled.display("Piece Missing on", f"{position}")
                        first_error = 1

            if errors:
                if error_detected == 0:
                    #SPEAKER: error detected please check display
                    error_detected = 1
                ...
                oled.display("Stability Check", "in progress")
    ...
    def check(self, square_name):
        pin = self.all_pins.get(square_name.lower())
        if pin is None:
            ChessOLED.display("Invalid Pin was checked")
            return "invalid"
        return "empty" if pin.value else "piece"
